...
<?php
	...
	$pfname_err = $pmname_err = $plname_err = $pusername =$pbday_err = $gender_err = $pemail_err= $pmobile_err= $address_err= $pid_err = "";
		...
		if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
			...
			$sql = "SELECT * FROM adm_ParentUser WHERE adm_prtId = ?";
			...
			if($stmt = mysqli_prepare($db, $sql)){
				...
				if(mysqli_stmt_execute($stmt)){
					...
					if(mysqli_num_rows($result) == 1){
						...
						$pfname = $row['adm_prtfname'];
						$pmname = $row['adm_prtmname'];
						$plname = $row['adm_prtlname'];
						$pbirthday = $row['adm_prtbday'];
						$pgender = $row['adm_prtgender'];
						$pmobile = $row['adm_prtmobile'];
		...
		if(isset($_POST['goEdit'])){
			...
			empty($pusername_err) &&empty($pgender_err)){
				...
				if($stmt = mysqli_prepare($db, $sql)){
					...
					if(mysqli_stmt_execute($stmt)){
						header("location: ParentView.php?id=$id");
						exit();
					} else {
						alert("Can't be done");
					}
				...
				<!-- Main -->
					<div id="main">
						<div class="inner">
							...
							<!-- Header -->
								<header id="header">
									<a href="" class="logo"><strong>Parent </strong>Update User</a>
									<ul class="icons"><?php
											...
											<tr id ="nodborder">
												<td>Personal Information<br><br>
													<div class="form-group">
														...
														<input type="text" name="fname" id="username" class="form-control <?php echo (!empty($pfname_err)) ? 'is-invalid': ''; ?>" value="<?php echo $row["adm_prtfname"];; ?>" placeholder="Enter First name">
												...
												<td><br><br>
													<div class="form-group">
														...
														<input type="text" name="mname" id="useremail"  class="form-control <?php echo (!empty($pmname_err)) ? 'is-invalid': ''; ?>" placeholder="Enter Middle Name"value="<?php echo $row["adm_prtmname"]; ?>">
												...
												<td><br><br>
													<div class="form-group">
														...
														<input type="text" name="lname" id="useremail"class="form-control <?php echo (!empty($plname_err)) ? 'is-invalid': ''; ?>" value="<?php echo $row["adm_prtlname"]; ?>" placeholder="Enter Last Name">
											...
											<tr>
												<td>
													<div class="form-group">		
														...
														<input type="date" name="bday" id="useremail" class="form-control  <?php echo (!empty($pbday_err)) ? 'is-invalid': ''; ?>" value="<?php echo $row["adm_prtbday"]; ?>" >
												...
												<td>
													<div class="form-group">	
															...
															<?php
																if($row['adm_prtgender'] == "male" || $row['adm_prtgender'] == "Male"){
												...
												<div class="form-group">		
														...
														<input type="text" name="mobile" id="useremail" class="form-control  <?php echo (!empty($pmobile_err)) ? 'is-invalid': ''; ?>" placeholder="Enter Mobile" value="<?php echo $row["adm_prtmobile"];; ?>">									
											...
											<tr>
												<td colspan = 3>
													<div class="form-group">
														...
														<input type="email" name="email" id="useremail" class="form-control <?php echo (!empty($email_err)) ? 'is-invalid': ''; ?>" value="<?php echo $row["adm_prtemail"]; ?>" placeholder="Enter Email" >
											...
											<tr>
												<td colspan = 3>Account Information<br><br>
													<div class="form-group">
														...
														<input type="text" name="username" id="useremail" class="form-control <?php echo (!empty($username_err)) ? 'is-invalid': ''; ?>" value="<?php echo $row["adm_prtusername"]; ?>" placeholder="Enter Username" >
